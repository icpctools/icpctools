...
public class BubbleOut {
	...
	public static void bubbleOut(TeamIntroPresentation.Position[] instPos, int teamInd2, int width, int height,
			double imageScaleFactor, long dt) {
		int teamInd = Math.min(teamInd2, instPos.length);
		...
		for (int i = 0; i < teamInd; i++) {
			...
			if (totalAdjust > adjustThreshold || minD > maxD) {
				...
				// System.out.println(String.format("%s %.3f adj %.3f %.2f scale %.2f",
				// p.label.substring(0, 10), d, totalAdjust, minD / maxD, imageScaleFactor));
			}
		}
	}
