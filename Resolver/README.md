...
The Resolver can work with contest data from one of
three different sources: a _Contest Package_ folder, a _Contest API_ source, or an _event feed_ file.
...
Alternatively the Resolver can obtain contest data from a Contest API server - for example
the Contest Data Server (CDS).
The Contest API provides URL access points for obtaining event feed and other contest data;
and starting the Resolver with the URL of a Contest API causes it to connect to the Contest API
and pull information dynamically.

The main reason for using a Contest Package or Contest API instead of a simple event feed file to provide
Resolver input is that these sources can provide additional information to the Resolver, including logos
or team photos. The sample image (above) shows the use of such logos.

Note: The Resolver expects the contest to be "_finalized_". "Finalizing" is an
operation performed in the CCS which indicates that the contest
has ended and contains the complete and final results.

#### Award Properties and Generating Awards

The Contest API defines the properties of an award: an id, citation, and the set of teams (or none) that
received the award. As stated above, the Resolver will work with awards created any spec-compliant tool.
In simple terms, the Resolver works by starting with the scoreboard at the contest freeze, resolving each
row in turn from the bottom to the top. Every time a row is fully resolved it will stop to display any
awards that team received.

In order to provide more control over the display, an additional property named `display_mode` is used
to customize how the resolver presents each award. The possible values of this property are:

- 'detail': Stop and show a separate screen with the team's photo and all awards given.
- 'pause': Stop and highlight the team, but do not show the detail screen.
- 'list': for awards given to multiple teams: show a scrolling list of all of the teams.
- 'photos': same as 'list', but show all of the team's photos on screen.
- 'ignore': skip the award entirely.

See [Spec Extensions](https://github.com/icpctools/icpctools/blob/main/doc/spec-extensions.md#award)
for full details. Any other tool can generate this property, or you can edit awards generated by another
tool (by editing the event feed, awards.json, or using the Award Generator) to customize the display.
...
The first parameter is a contest source. It may be a contest package folder,
a URL to a Contest API server, or an event feed file. If the parameter is a contest URL,
...
```
resolver.bat c:\contests\pacnw
```
The above command runs the Resolver in "stand-alone" mode, taking its input from the specified
contest package folder.
It includes whatever additional data it can find (for example, team pictures and/or
...
If there are no awards assigned in a contest, the resolver will automatically create some:
- 4 gold, 4 silver, and 4 bronze medals
- First to solve for each solved problem
- First team in every group

This provides some basic awards to test with, but it is expected that most contests will assign
their own awards prior to running the Resolver - either directly in the CCS, by manually editing
the event feed or awards.json, or by using the included _Award Generator (AG)_. The Award Generator
can be used in two ways: by passing command line parameters to the _awards_ script, or by running
the script with no parameters, which will launch a separate graphical user interface (GUI) that
allows interactive award configuration.
...
This screen is used to select where to load the contest to generate awards.
The interactive Award Generator is capable of operating with data obtained from one of
three different sources: a Contest Package, a Contest API, or an event feed file.

Selecting the desired option and pressing the "Load" button 
will load the data into the AG and then display the 
Award Generator main screen, as shown below:

![Award Generator Main Screen](docs/AwardGeneratorMainScreen.png)

In the above screen the Award Generator has been loaded with the finalized data from the 2015 ICPC World Finals.
...
*Award templates are an ICPC Tools spec extension and subject to change at any time.*

At many contests we know the awards that will be applied prior to the start, and they will potentially
be applied many times between system tests, practices, and the actual contest. By creating a template
and applying it to each contest you can save time and reduce the chance of human error.

An award template looks just like a set of awards with additional properties, specified in either ndjson
or json array (i.e. awards.json) format. Each award specifies an id to specify the pattern to apply, any
additional properties required by the pattern, and a citation. (some awards have default citations)
When the template is applied it creates the corresponding award(s) in the contest, with the citation
and team_ids that the award(s) apply to.

Award template patterns:

| Award id           | Parameters  | Description
| :----------------- | :---------- | :----------
...
| honors             | see below   | A single award for honourable mentions.

Additional parameters for honor awards. Awards are given to all the teams that fall between the top (inclusive) and
bottom (exclusive) parameter. For example, percentileTop = 1 and percentileBottom = 25 will assign the award to the
top 25% of teams in the contest.
...
Example template used at World Finals:
```json
{"id":"winner","citation":"48th ICPC World Finals Champion"}
...
{"id":"group-winner-*","citation":"ICPC World Finals {0} Champion"}
{"id":"group-winners","citation":"ICPC World Finals Division Champions"}
```
...
and will save a new augmented event feed file containing the configured awards in JSON format in the specified file.
...
If on the other hand the Resolver is obtaining its input from a local Contest Package
or via a connection to a CDS,
the augmented event feed file must be copied to the local Contest Package folder or to the Contest Package being hosted by the CDS,
