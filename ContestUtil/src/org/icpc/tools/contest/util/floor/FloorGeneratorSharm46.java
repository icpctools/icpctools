...
public class FloorGeneratorSharm46 extends FloorGenerator {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < 144; i++) {
				...
				floor.createTeam(46000 + n, xx, yy, (y % 2 == 0) ? FloorMap.N : FloorMap.S);
			}
			...
			floor.createAisle(left + taw, bottom - tad * 2 - aisle, left + taw, bottom); // vertical
			floor.createAisle(left + taw, bottom, left, bottom - tad * 2 - aisle); // diagonal

			ITeam t = createAdjacentTeam(floor, 46001, -1, 0, -taw);
			floor.makeSpare(t);

			t = createAdjacentTeam(floor, 46032, -1, 0, -taw);
			floor.makeSpare(t);
